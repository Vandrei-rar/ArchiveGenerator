...
public class gravar {
	...
	public void gravando() throws IOException {
		...
		if(verArq.exists()) { // Verifica se o arquivo existe.
			...
			PrintWriter escrevaArq = new PrintWriter(arq); // Escreve os dados no arquivo indicado em -> PrintWriter(ArquivoCriadoAqui).
			escrevaArq.printf("%s", "Mensagens"); // Cria cabeçalho
			
			escrevaArq.printf("%s", msg); // Importante¹!! Como no C é necessario indicar ..(%tipoVariavel", variavel) para esse caso e semelhantes.
